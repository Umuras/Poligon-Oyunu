...
public class AtesEtme2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.S) && mermi > 0 && Time.time > siradakiates && !reload)
        {
            ates = true;
            SesKaynak.Play();
            SesKaynak.clip = FireSound;
            GetComponent<Animation>().Play("Tabanca");
        ...
        if (ates)
        {
            ...
            if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit, menzil))
            {
                if (hit.transform.tag == "dusman")
                {
                    ...
                    {
                        hit.transform.gameObject.GetComponent<Hedef>().can -= 40;
                    }
